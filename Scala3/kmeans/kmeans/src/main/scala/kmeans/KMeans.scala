...
class KMeans extends KMeansInterface:
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = points.groupBy(findClosest(_, means))
  ...
  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
    val distanceSquare = oldMeans.zip(newMeans).map{case (p1, p2) => p1.squareDistance(p2)}.foldLeft(0.0)(_+_)
    distanceSquare <= eta*eta
