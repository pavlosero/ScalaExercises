...
trait Solver extends GameDef:
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = (for n <- b.legalNeighbors yield (n._1, n._2::history)).to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    def fromAcc(init: LazyList[(Block, List[Move])],
                expl: Set[Block],
                acc: LazyList[(Block, List[Move])]): LazyList[(Block, List[Move])] = {
      val frontier = for {
        (block, history) <- init
        next <- newNeighborsOnly(neighborsWithHistory(block, history), expl)
      } yield next
      if (init.isEmpty)
        acc
      else
        fromAcc(frontier, expl ++ (frontier map(_._1)), init #::: acc)
    }


    fromAcc(initial, explored, LazyList())
