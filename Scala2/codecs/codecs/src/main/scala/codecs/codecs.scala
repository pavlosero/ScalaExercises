...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromFunction { jnum => jnum match {
      case Json.Num(x) => if (x.isValidInt) Some(x.toInt) else None
      case _ => None
    }}
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
        jArr  => { jArr match {
          case Json.Arr(list) => {
            val options = list.map(decoder.decode)
            if (options.contains(None)) None
            else Some(options.map {case Some(x) => x})
          }
          case _ => None
        }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction { case Json.Obj(map) => decoder.decode(map.get(name).getOrElse(Json.Null)).get }
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] =
    ObjectEncoder.field[List[Person]]("people").transform[Contacts](cc => cc.people)
