...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().toList.groupBy(e=>e).map({case (c, s) => (c, s.length)}).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = List() :: (for {
    (c, t) <- occurrences
    i <- 1 to t
    rest <- combinations(occurrences.filter(occ => occ._1 > c))
  } yield List((c, i)) ++ rest)
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.toMap.foldLeft(Map[Char, Int]())((acc, occ) => {
    acc.updated(occ._1, occ._2 - y.toMap.withDefaultValue(0)(occ._1))
  }).filter(occ => occ._2 > 0).toList.sortBy(occ => occ._1)
    ...
    def anagrams(occs: Occurrences): List[Sentence] = occs match {
      ...
      case _ =>
        for {
          ...
          rest <- anagrams(subtract(occs, combination))
        } yield word :: rest
    }
